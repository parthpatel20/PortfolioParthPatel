...
const PersonalDetail = () => {
    ...
    return (<div><CardMedia

        className={(matches) ? (lg) ? classes.coverimgLg : classes.coverimg : classes.coverimgSm}
        image={contactInfo.image}
    >
        <Container>
            <label className={classes.title}> {contactInfo.name}</label><br></br>
            <h4 className={classes.titleSmall}>{contactInfo.position}</h4>
            <p className={classes.titleSmall}>To work in an organization where my technical skills, experience, creativity and knowledge can be utilized at the fullest.</p>
        </Container>
        <Container >
